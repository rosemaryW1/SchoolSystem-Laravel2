    ...
    <div class="create">
        ...
        <div class="flex items-center justify-between mb-6">
            <div><!-- Log on to codeastro.com for more projects -->
                <h2 class="text-black-700 uppercase font-bold">Attendance for class :- {{ $class->class_name }}</h2>
            </div>
            <div class="flex flex-wrap items-center">
                <a href="{{ route('home') }}" class="bg-orange-700 text-white text-sm uppercase py-2 px-4 flex items-center rounded">
        ...
        <div class="w-full mt-8 bg-white rounded">
            ...
            <div class="w-full px-6 py-6">
                <div class="flex items-center bg-black-600 rounded-tl rounded-tr">
                ...
                <form action="{{ route('teacher.attendance.store') }}" method="POST">
                    @foreach ($class->students as $student)
                        <div class="flex items-center justify-between border border-gray-200">
                            @csrf
                            <div class="w-4/12 text-sm text-left text-black-600 py-2 px-4 font-semibold">{{ $student->user->name }}</div>
                            <div class="w-3/12 text-sm text-left text-black-600 py-2 px-4 font-semibold">{{ $student->roll_number }}</div>
