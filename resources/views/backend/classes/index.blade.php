...
@section('content')
    <div class="roles-permissions">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-orange-700 uppercase font-bold">Classes</h2>
            </div>
        ...
        <div class="mt-8 bg-white rounded border-b-4 border-orange-300">
            ...
            @foreach ($classes as $class)
                <div class="flex flex-wrap items-center text-orange-700 border-t-2 border-l-4 border-r-4 border-orange-300">
                    <div class="w-1/12 px-4 py-3 text-sm font-semibold text-orange-600 tracking-tight">{{ $class->class_numeric }}</div>
                    <div class="w-2/12 px-4 py-3 text-sm font-semibold text-orange-600 tracking-tight">{{ $class->class_name }}</div>
                    <div class="w-1/12 px-4 py-3 text-sm font-semibold text-orange-600 tracking-tight">
                        <span class="bg-orange-200 text-sm mr-1 mb-1 px-2 font-semibold border rounded-full">
                            {{ $class->students_count }}
                        </span>
                    </div>
                    <div class="w-4/12 px-4 py-3 text-sm text-orange-600 text-center tracking-tight">
                        @foreach ($class->subjects as $subject)
                            <span class="bg-orange-200 text-sm mr-1 mb-1 px-2 font-semibold border rounded-full">{{ $subject->subject_code }}</span>
                        @endforeach
                    </div>
                    <div class="w-2/12 px-4 py-3 text-sm font-semibold text-orange-600 tracking-tight">{{ $class->teacher->user->name ?? '' }}</div>
